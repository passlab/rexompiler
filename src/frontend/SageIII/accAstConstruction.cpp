...
static std::vector<std::pair<std::string, SgNode*> >* acc_variable_list = &omp_variable_list;
...
static SgExpression* parseAccArraySection(std::pair<SgPragmaDeclaration*, OpenACCDirective*>, OpenACCClauseKind, std::string);
...
SgOmpClause* convertOpenACCClause(SgStatement* directive, std::pair<SgPragmaDeclaration*, OpenACCDirective*> current_OpenACCIR_to_SageIII, OpenACCClause* current_acc_clause) {
    ...
    if (current_expressions->size() != 0) {
        std::vector<std::string>::iterator iter;
        for (iter = current_expressions->begin(); iter != current_expressions->end(); iter++) {
            parseAccArraySection(current_OpenACCIR_to_SageIII, current_acc_clause->getKind(), *iter);
        }
    }

    SgOmpClause::upir_data_mapping_enum data_mapping_type = SgOmpClause::e_upir_data_mapping_unspecified;
    ...
    for (std::vector<std::pair<std::string, SgNode*> >::iterator iter = acc_variable_list->begin(); iter != acc_variable_list->end(); iter++) {
        SgInitializedName* symbol = isSgInitializedName((*iter).second);
        ROSE_ASSERT(symbol != NULL);
        SgVariableSymbol* variable_symbol = isSgVariableSymbol(symbol->get_symbol_from_symbol_table());
    ...
    for (size_t i = 0; i < variables.size(); i++) {
        ...
        std::list<std::list<SgExpression *>> upir_section = upir_data_item->get_section();
        std::vector<std::pair<SgExpression *, SgExpression *>> sections = array_dimensions[variable_symbol];
        ...
        upir_data_item->set_sharing_property(SgOmpClause::e_upir_data_sharing_shared);
...
SgExpression* parseAccArraySection(std::pair<SgPragmaDeclaration*, OpenACCDirective*> current_OpenACCIR_to_SageIII, OpenACCClauseKind clause_kind, std::string expression) {
    // special handling for omp declare simd directive
    // It may have clauses referencing a variable declared in an immediately followed function's parameter list
    bool look_forward = false;
    std::string expr_string = std::string() + "array_section (" + expression + ")\n";
    SgExpression* sg_expression = parseArraySectionExpression(current_OpenACCIR_to_SageIII.first, look_forward, expr_string.c_str());
