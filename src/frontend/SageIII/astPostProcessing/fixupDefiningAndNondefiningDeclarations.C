...
void fixupAstDefiningAndNondefiningDeclarations( SgNode* node )
  ...
  // DQ (3/4/2007): temp debugging code to check where defining and non-defining declarations were being set equal!
     ...
     MLOG_WARN_C("astPostProcessing", "saved_declaration = %p saved_declaration->get_definingDeclaration() = %p saved_declaration->get_firstNondefiningDeclaration() = %p \n",
...
FixupAstDefiningAndNondefiningDeclarations::visit ( SgNode* node )
...
#if 0
      MLOG_WARN_C("astPostProcessing", "In FixupAstDefiningAndNondefiningDeclarations::visit(node = %p = %s) \n",node,node->class_name().c_str());
#endif
  ...
  // if ( isSgFunctionParameterList(node) != NULL || isSgVariableDeclaration(node) != NULL || isSgVariableDefinition(node) != NULL)
     if ( isSgFunctionParameterList(node) != NULL || isSgVariableDefinition(node) != NULL)
        {
    	  //MLOG_WARN_C("astPostProcessing", "Skipping case of SgFunctionParameterList or SgVariableDeclaration in FixupAstDefiningAndNondefiningDeclarations::visit() node = %p = %s \n",node,node->class_name().c_str());
          return;
        }
     ...
     if (declaration != NULL)
...
#if 0
          MLOG_WARN_C("astPostProcessing", "In FixupAstDefiningAndNondefiningDeclarations: node = %p = %s definingDeclaration = %p firstNondefiningDeclaration = %p \n",
       ...
       // FMZ (6/8/2008): caused core dump when read in a .rmod file
          if (definingDeclaration == NULL && firstNondefiningDeclaration == NULL)
             {
               MLOG_ERROR_C("astPostProcessing", "Error: declaration = %p = %s definingDeclaration         = %p \n",declaration,declaration->sage_class_name(),definingDeclaration);
               MLOG_ERROR_C("astPostProcessing", "Error: declaration = %p = %s firstNondefiningDeclaration = %p \n",declaration,declaration->sage_class_name(),firstNondefiningDeclaration);
             }
          ...
          if (definingDeclaration == NULL)
            ...
            // DQ (10/10/2006): This should apply only to SgClassDeclarations (else fails later for cases below).
               ...
               if ( (firstNondefiningClassDeclaration != NULL) && (firstNondefiningClassDeclaration->get_definingDeclaration() != NULL) )
                  ...
                  // MLOG_WARN_C("astPostProcessing", "In FixupAstDefiningAndNondefiningDeclarations::visit(): declaration = %p firstNondefiningClassDeclaration->get_definingDeclaration() = %p \n",declaration,firstNondefiningClassDeclaration->get_definingDeclaration());
                  }
...
#if 0
                    	MLOG_WARN_C("astPostProcessing", "We have identified an inconsistantcy where the defining function pointer has not be uniformally setup \n");
#endif
            ...
            // DQ (4/10/2016): Output a warning when this happens.
               if (firstNondefiningDeclaration == NULL)
                 ...
                 // DQ (11/5/2016): Comment this out to avoid output spew in Fortran test codes (see Fortran_tests/mpi_f08_interfaces_test.f03, I think).
            	   MLOG_WARN_C("astPostProcessing", "Warning: declaration exists with firstNondefiningDeclaration == NULL: declaration = %p = %s \n",declaration,declaration->class_name().c_str());
#endif
                  }
            // ROSE_ASSERT(firstNondefiningDeclaration != NULL);
            // if (firstNondefiningDeclaration    ->get_definingDeclaration() != NULL && declaration->get_definingDeclaration() == NULL)
               if (firstNondefiningDeclaration != NULL && firstNondefiningDeclaration->get_definingDeclaration() != NULL && declaration->get_definingDeclaration() == NULL)
                 ...
                 // We have identified an inconsistantcy where the defining function pointer has not be uniformally setup.
#if 0
            	   MLOG_WARN_C("astPostProcessing", "Warning: Fixup defining declarations: We have identified an inconsistantcy where the defining declaration has not be uniformally setup \n");
#endif
                    declaration->set_definingDeclaration(firstNondefiningDeclaration->get_definingDeclaration());
                  }
             }
               ...
               if (nondefiningClassDeclaration != NULL)
                  {
                    if (nondefiningClassDeclaration->get_fixupScope() == true)
                       {
                    	MLOG_WARN_C("astPostProcessing", "nondefiningClassDeclaration = %p = %s = %s \n",
                 ...
                 // DQ (10/10/2006): added assertion that "nondefiningClassDeclaration->get_scope() != NULL"
                    ...
                    if (nondefiningClassDeclaration->get_fixupScope() == true)
                       {
                    	MLOG_ERROR_C("astPostProcessing", "Error: nondefiningClassDeclaration = %p = %s get_fixupScope() == false \n",nondefiningClassDeclaration,nondefiningClassDeclaration->get_name().str());
                       }
               ...
               if (!equivalent_scopes)
                 ...
                 // pointer.
                    if (symbolToMove != NULL)
                       {
#if 0
                    	MLOG_WARN_C("astPostProcessing", "Found a symbol to move as we reset the scope of firstNondefiningDeclaration = %p = %s to definingScope = %p \n",
                              firstNondefiningDeclaration,firstNondefiningDeclaration->class_name().c_str(),definingScope);
                    	MLOG_WARN_C("astPostProcessing", "Removing symbol = %p from scope = %p \n",symbolToMove,nondefiningScope);
#endif
                      ...
                      // definition where they are placed plus the global copy of the namespace definition.
                         if (nondefiningScope->symbol_exists(symbolToMove) == true)
                            ...
                            {
#if 0
                        	   MLOG_WARN_C("astPostProcessing", "AST Fixup: this symbol does not exist \n");
#endif
                            }
                      ...
                      // already exist in the definingScope's symbol table.  (Confirmed to be true).
                         if (definingScope->symbol_exists(symbolToMove->get_name(),symbolToMove) == false)
                            {
#if 0
                        	 MLOG_WARN_C("astPostProcessing", "calling insert symbolToMove = %p = %s = %s into newScope = %p \n",
                           ...
                           // ROSE_ASSERT (definingScope->symbol_exists(symbolToMove) == false);
                              if (definingScope->symbol_exists(symbolToMove) == false)
                                 {
#if 0
                            	  MLOG_WARN_C("astPostProcessing", "After existence tesst: calling insert symbolToMove = %p = %s = %s into newScope = %p = %s \n",
                                ...
                                // Not sure if this is an error or not.
                                	MLOG_WARN_C("astPostProcessing", "Warning: skipped insert of symbolToMove = %p = %s = %s into newScope = %p (symbolToMove already exists) \n",
                                        symbolToMove,symbolToMove->class_name().c_str(),symbolToMove->get_name().str(),definingScope);
#endif
                                ...
                                // DQ (3/5/2007): however, we should check to make sure that the parent is set properly
                                   ...
                                   if (symbolToMove->get_parent() != definingScope->get_symbol_table())
                                      {
#if 0
                                	   MLOG_WARN_C("astPostProcessing", "Resetting the parent of the symbolToMove = %p = %s = %s from %p = %s to %p (symbol table) \n",
...
#if 0
                    MLOG_WARN_C("astPostProcessing", "Resetting the scope of firstNondefiningDeclaration = %p = %s from %p = %s to %p = %s \n",
                 ...
                 // DEBUG_SAGE_ACCESS_FUNCTIONS_ASSERTION in sage3.h indicate this is required.
                    if (firstNondefiningDeclaration->get_scope() != NULL)
                       {
                    	MLOG_WARN_C("astPostProcessing", "Note in FixupAstDefiningAndNondefiningDeclarations::visit(): overwriting firstNondefiningDeclaration = %p firstNondefiningDeclaration->get_scope() = %p with NULL before assignment to definingScope = %p \n",
...
#endif
#if 0
                    MLOG_WARN_C("astPostProcessing", "Calling set_scope: firstNondefiningDeclaration = %p = %s = %s to definingScope = %p = %s = %s \n",
                      ...
                      //     alternatively, firstNondefiningDeclarationScope = isSgScopeStatement(SgAdaDiscriminatedTypeDecl::get_parent()) could be used
                       if (  (!isSgTypedefDeclaration(firstNondefiningDeclaration->get_parent())) ) {
                          ...
                          if (nonDefDclParent != NULL) {
                        	  MLOG_WARN_C("astPostProcessing", "     firstNondefiningDeclaration->get_parent() = %s \n",nonDefDclParent->class_name().c_str());
                          }
#if 0
                         ...
                         // DQ (3/4/2009): This test fails for the AST copy mechanism on test2005_163.C
                          MLOG_WARN_C("astPostProcessing", "Warning: failing test: firstNondefiningDeclarationScope != NULL \n");
                          MLOG_WARN_C("astPostProcessing", "This test fails for the AST copy mechanism on test2005_163.C \n");
                       }
                    }
...
// #if PRINT_DEVELOPER_WARNINGS
#if 0
                        	  MLOG_WARN_C("astPostProcessing", "Resetting the parent of the templateClassInstantiation = %p = %s to its SgClassType \n",
                           ...
                           // DQ (12/22/2019): Comment out output spew from multiple files on the command line seeing the same class definitions (no longer shared across files).
                        	  MLOG_WARN_C("astPostProcessing", "In FixupAstDefiningAndNondefiningDeclarations::visit() templateClassInstantiation->get_parent() = %p \n",templateClassInstantiation->get_parent());
                              if (templateClassInstantiation->get_parent() != NULL)
                            	  MLOG_WARN_C("astPostProcessing", "In FixupAstDefiningAndNondefiningDeclarations::visit() templateClassInstantiation->get_parent() = %s \n",templateClassInstantiation->get_parent()->class_name().c_str());
#endif
...
#if 0
                                // DQ (12/22/2019): Comment out output spew from multiple files on the command line seeing the same class definitions (no longer shared across files).
                                    MLOG_WARN_C("astPostProcessing", "templateClassInstantiation parent is already set, skip resetting it to SgClassType \n");
#endif
                                 }
                    ...
                    if (lookForDeclarationInAssociatedScope == true && firstNondefiningDeclarationScope != NULL)
                      ...
                      // Make sure that the statement is in the list...
                         if (std::find(statementList.begin(),statementList.end(),firstNondefiningDeclaration) == statementList.end())
                            {
                        	 MLOG_WARN_C("astPostProcessing", "##### WARNING: in FixupAstDefiningAndNondefiningDeclarations::visit() statement = %p = %s not in child list of scope = %p = %s \n",
                                   firstNondefiningDeclaration,firstNondefiningDeclaration->class_name().c_str(),
                              ...
                              if (classDeclaration != NULL)
                                 {
                            	  MLOG_WARN_C("astPostProcessing", " --- classDeclaration = %p name = %s \n",classDeclaration,classDeclaration->get_name().str());
                                 }
#if 0
...
#if 0
               MLOG_WARN_C("astPostProcessing", "Testing associated scope of declaration statment (node = %p = %s = %s) \n",
                    declaration,declaration->sage_class_name(),SageInterface::get_name(declaration).c_str());
               MLOG_WARN_C("astPostProcessing", "definingDeclaration->get_scope() = %p firstNondefiningDeclaration->get_scope() = %p \n",
     ...
     switch (node->variantT())
        {
          case V_SgEnumDeclaration:
            ...
            // MLOG_WARN_C("astPostProcessing", "In FixupAstDefiningAndNondefiningDeclarations::visit(): declaration = %p definingDeclaration = %p \n",declaration,definingDeclaration);
               if (declaration != definingDeclaration)
                  {
                 // MLOG_WARN_C("astPostProcessing", "Note: in FixupAstDefiningAndNondefiningDeclarations::visit(), enum declaration not a defining declaration \n");
...
#if 0
            	 MLOG_WARN_C("astPostProcessing", "In AST Fixup: declaration %p = %s definingDeclaration = %p \n",declaration,declaration->class_name().c_str(),definingDeclaration);
#endif
               if (declaration != definingDeclaration)
                  {
#if 0
            	    MLOG_WARN_C("astPostProcessing", "Warning in AST Fixup: declaration %p = %s used to set definingDeclaration = %p \n",
                         declaration,declaration->class_name().c_str(),definingDeclaration);
#endif
                 ...
                 // MLOG_WARN_C("astPostProcessing", "In FixupAstDefiningAndNondefiningDeclarations: set_definingDeclaration of %p to %p \n",declaration,declaration);
...
#if 0
               MLOG_WARN_C("astPostProcessing", "declaration = %p definingDeclaration = %p \n",declaration,definingDeclaration);
#endif
               ...
               if (firstNondefiningDeclaration != NULL)
                 ...
                 // IR nodes have an explicit scope data member.
                    if (declaration->hasExplicitScope() == true)
                       {
#if 1
                    	 MLOG_WARN_C("astPostProcessing", "Resetting scope (declaration->hasExplicitScope() == true) of declaration = %p = %s old scope = %p = %s new scope = %p = %s \n",
                              declaration,declaration->class_name().c_str(),
...
#if 0
                    MLOG_WARN_C("astPostProcessing", "declaration->get_scope() = %p firstNondefiningDeclaration->get_scope() = %p \n",
                 ...
                 // MLOG_WARN_C("astPostProcessing", "WARNING: Fundamental problem in get_scope function! \n");
          ...
          case V_SgTemplateInstantiationMemberFunctionDecl:
            ...
            // it may or may not point to the current declaration (since multiple declarations may be non-defining).
               if (firstNondefiningDeclaration != NULL)
                 ...
                 // MLOG_WARN_C("astPostProcessing", "Warning: declaration %p = %s has no firstNondefiningDeclaration = %p \n",
                 //      declaration,declaration->sage_class_name(),firstNondefiningDeclaration);
                  }
            // ROSE_ASSERT(firstNondefiningDeclaration != NULL);
            ...
            // firstNondefiningDeclaration should never be the same as the definingDeclaration (if they are non-null)
               if (firstNondefiningDeclaration != NULL && firstNondefiningDeclaration == definingDeclaration)
                  {
#if 0
            	   MLOG_WARN_C("astPostProcessing", "AST Fixup: declaration = %p = %s firstNondefiningDeclaration == definingDeclaration at: \n",
               ...
               if (firstNondefiningDeclaration == definingDeclaration)
                  {
                    MLOG_WARN_CXX("astPostProcessing") << "firstNondefiningDeclaration == definingDeclaration failed for node of"
			    << node->sage_class_name() << "/" << node->unparseToString() << "\n";
               ...
               if (templateMemberFunction != NULL)
                 ...
                 // DQ (3/4/2007): This can cause the defining and non-defining declarations to be the same, which is an violation of AST consistancy rules.
                    if (templateDeclaration->get_definingDeclaration() == NULL)
                       {
                    	 MLOG_WARN_C("astPostProcessing", "Setting previously unset definingDeclaration on SgTemplateDeclaration to itself  templateDeclaration = %p \n",templateDeclaration);
                 ...
                 // template declaration to not be present (since it can be nested in another outer template class declaration).
                    if (templateDeclaration->get_definingDeclaration() == NULL && templateDeclaration->get_firstNondefiningDeclaration() == NULL)
                       {
                         MLOG_WARN_C("astPostProcessing", "Error: there should be at least a defining or non-defining template declaration available templateDeclaration = %p \n",templateDeclaration);
                       }
...
// #ifdef ROSE_DEBUG_NEW_EDG_ROSE_CONNECTION
            ...
            // DQ (9/24/2007): This is an error reported by Andreas, fix it after Gergos checkin.
               ...
               if ( (memberFunctionDeclaration != NULL) && (memberFunctionDeclaration->get_associatedClassDeclaration() == NULL) )
                  {
#if 0
            	   MLOG_WARN_C("astPostProcessing", "Error: memberFunctionDeclaration->get_associatedClassDeclaration() == NULL for memberFunctionDeclaration = %p = %s \n",
     ...
     if (declaration != NULL)
        {
    	 MLOG_WARN_C("astPostProcessing", "Leaving FixupAstDefiningAndNondefiningDeclarations: node = %p = %s definingDeclaration = %p firstNondefiningDeclaration = %p \n",
               node,node->class_name().c_str(),definingDeclaration,firstNondefiningDeclaration);
       ...
       // DQ (3/4/2007): temp debugging code to check where defining and non-defining declarations were being set equal!
          ...
          if (declaration == (SgDeclarationStatement*)(0x2a96f96480))
             {
        	  MLOG_WARN_C("astPostProcessing", "Setting the saved_declaration to %p \n",declaration);
