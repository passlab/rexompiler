...
int patchUpFirstprivateVariables(SgFile*  file)
{
  ...
  return result;
    ...
    int patchUpImplicitMappingVariables(SgFile* file)
    {
        int result = 0;
        ROSE_ASSERT(file != NULL);
        Rose_STL_Container<SgNode*> node_list = NodeQuery::querySubTree(file, V_SgOmpParallelStatement);
        ...
        for (iter = node_list.begin(); iter != node_list.end(); iter++)
        {
            ...
            // Find all variable references from the task's body
            Rose_STL_Container<SgNode*> ref_list = NodeQuery::querySubTree(body, V_SgVarRefExp);
