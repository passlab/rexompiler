...
//! makeDataSharingExplicit() can call some of existing functions for some work
//! in OmpSupport namespace by Hongyi 07/16/2012
//! TODO: add a function within the OmpSupport namespace, the function should
//! transform the AST, so all variables' data-sharing attributes are explicitied
//! represented in the AST. ROSE has dedicated AST nodes for OpenMP directives
...
//! A driver to traverse AST trees and invoke individual translators for OpenMP
//! constructs, (not in use) Postorder is preferred.
...
//! generated code block in which the variable handling statements will be
//! inserted. Original loop upper bound is needed for implementing lastprivate
//! (check if it is the last iteration). withinAcceleratorModel means if we only
//! translate private() variables, used to support accelerator model
...
//! Collect variables from a particular type of OpenMP clauses associated with
//! an omp statement: private, reduction, etc
ROSE_DLL_API SgInitializedNamePtrList
collectClauseVariables(SgStatement *clause_stmt, const VariantT &vt);

//! Collect variables from given types of OpenMP clauses associated with an omp
//! statement: private, reduction, etc
ROSE_DLL_API SgInitializedNamePtrList
collectClauseVariables(SgStatement *clause_stmt, const VariantVector &vvt);

//! Collect expression from given types of OpenMP clauses associated with an omp
//! statement: private, reduction, etc
...
//! Replace references to oldVar within root with references to newVar, return
//! the number of references replaced.
...
//! Check if an omp for/do loop use static schedule or not, including: default
//! schedule, or schedule(static[,chunk_size])
...
//! Patch up private variables for omp for of entire file. The reason is that
//! loop indices should be private by default and this function will make this
//! explicit
...
//! attributes and this function will make the firstprivate variable of omp task
//! explicit.
...
//! avoid duplicated elements. No input parameters are needed since it finds
//! match from memory pools
std::set<SgInitializedName *> collectThreadprivateVariables();

//! Special handling when trying to build and insert a variable declaration into
//! a BB within Fortran OpenMP code
...
//! Check if a variable is a threadprivate variable. It will search for all
//! threadprivate directives to find the answer.
ROSE_DLL_API bool isThreadprivate(SgSymbol *var);

//! Return the data sharing attribute type of a variable, specified as a symbol
//! and an anchor node (Must be the inner most node associated with the variable
//! reference, e.g. a SgVarRefExp, SgVariableDeclaration, etc) Possible returned
//! values include: e_shared, e_private,  e_firstprivate,  e_lastprivate,
//! e_reduction, e_threadprivate, e_copyin, and e_copyprivate.
