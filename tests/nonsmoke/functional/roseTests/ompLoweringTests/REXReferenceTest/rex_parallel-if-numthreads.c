...
#include <omp.h>
#include "rex_kmp.h" 
static void OUT__1__4997__(int *__global_tid,int *__bound_tid,void *__out_argv);
static void OUT__2__4997__(int *__global_tid,int *__bound_tid,void *__out_argv);
...
int main(int argc,char **argv)
{
  ...
  int i = 0;
  if (i == 0) 
    __kmpc_fork_call(0,1,OUT__2__4997__,0);
   else {
    int __global_tid = __kmpc_global_thread_num(0);
    OUT__2__4997__(&__global_tid,0,0);
  }
  if (i != 0) {
    __kmpc_push_num_threads(0,(__kmpc_global_thread_num(0)),3);
    __kmpc_fork_call(0,1,OUT__1__4997__,0);
  }
   else {
    int __global_tid = __kmpc_global_thread_num(0);
    OUT__1__4997__(&__global_tid,0,0);
  }
  return 0;
}
